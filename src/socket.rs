...
impl Socket {
    ...
    pub fn dccp_service(&self) -> io::Result<u32> {
        unsafe { getsockopt(self.as_raw(), sys::SOL_DCCP, sys::DCCP_SOCKOPT_SERVICE) }
    }
    ...
    pub fn dccp_set_server_timewait(&self, hold_timewait: bool) -> io::Result<()> {
        unsafe {
            setsockopt(
                self.as_raw(),
                sys::SOL_DCCP,
                sys::DCCP_SOCKOPT_SERVER_TIMEWAIT,
    ...
    pub fn dccp_send_cscov(&self) -> io::Result<u32> {
        unsafe {
            getsockopt(self.as_raw(), sys::SOL_DCCP, sys::DCCP_SOCKOPT_SEND_CSCOV)
        }
    }
    ...
    pub fn dccp_set_send_cscov(&self, level: u32) -> io::Result<()> {
        unsafe {
            setsockopt(self.as_raw(), sys::SOL_DCCP, sys::DCCP_SOCKOPT_SEND_CSCOV, level)
        }
    }
    ...
    pub fn dccp_qpolicy_txqlen(&self) -> io::Result<u32> {
        unsafe {
            getsockopt(self.as_raw(), sys::SOL_DCCP, sys::DCCP_SOCKOPT_QPOLICY_TXQLEN)
        }
    }
    ...
    pub fn dccp_set_qpolicy_txqlen(&self, qlen: u32) -> io::Result<()> {
        unsafe {
            setsockopt(self.as_raw(), sys::SOL_DCCP, sys::DCCP_SOCKOPT_QPOLICY_TXQLEN, qlen)
        }
    }
